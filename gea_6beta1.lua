...
if (not GEA) then
  ...
  GEA.options = {
    ...
    open      = {name="Open", 
                    ...
                    action=function(id, value) if (not id) then id = GEA.currentMainId end if (not value) then value = 0 end GEA.options.value.getValue(id, value) end
                },
    close     = {name="Close",
                    math=true,
                    control=function(id) return GEA.options.open.control(id) end,
                    getValue=function(id) return GEA.options.value.getValue(id) end,
                    action=function(id, value) if (not id) then id = GEA.currentMainId end if (not value) then value = 100 end GEA.options.value.getValue(id, value) end
                },
    stop      = {name="Stop", 
                    control=function(id) return GEA.options.number.control(id) end,
  ...
  GEA.checkDates = function(from, to)
    ...
    to = to or from
    local tabfrom, tabto = tools.split(from:gsub("-", "/"):gsub("%.", "/"), "/"), tools.split(to:gsub("-", "/"):gsub("%.", "/"), "/")
    if (#tabfrom < 3) then tabfrom[3] = os.date("%Y") end
		if (#tabto < 3) then tabto[3] = os.date("%Y") end
    from = string.format ("%04d", tabfrom[3]) ..string.format ("%02d", tabfrom[2])..string.format ("%02d", tabfrom[1])
		to = string.format ("%04d", tabto[3]) ..string.format ("%02d", tabto[2])..string.format ("%02d", tabto[1])
  ...
  GEA.insert = function(t, v, entry)
    ...
    if (action == "maxtime") then entry.maxtime = v.args[1] entry.repeating = true return end
  ...
  GEA.init = function()
    ...
    if (#GEA.entries>0) then
      ...
      if (GEA.source.type == "autostart") then tools.info(GEA.trad.gea_stopped_auto) end
    end
  end
